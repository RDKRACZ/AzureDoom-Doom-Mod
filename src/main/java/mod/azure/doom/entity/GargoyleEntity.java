...
public class GargoyleEntity extends DemonEntity implements IAnimatable, IFlyingAnimal {
	...
	private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
		if (!this.dataManager.get(ATTACKING) && !this.onGround && !(this.dead || this.getHealth() < 0.01)) {
	...
	protected void registerAttributes() {
		...
		this.getAttribute(SharedMonsterAttributes.FLYING_SPEED).setBaseValue(2.25D);
	...
	public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason,
		...
		if (spawnDataIn == null) {
			spawnDataIn = new GargoyleEntity.GroupData(worldIn.getRandom()
