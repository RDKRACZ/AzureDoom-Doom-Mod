...
public class BarenBlastItem extends ShootableItem {
	...
	public BarenBlastItem setItemReference(RegistryObject<Item> refIn) {
		this.ref = refIn;
		return this;
	}
		...
		if (livingEntityIn instanceof PlayerEntity) {
			...
			if (!itemstack.isEmpty() || flag) {
				if (itemstack.isEmpty()) {
					itemstack = new ItemStack(DoomItems.BARENBLAST.get());
				}

				boolean flag1 = playerentity.abilities.isCreativeMode || (itemstack.getItem() instanceof BarenBlastItem
						&& ((BarenBlastItem) itemstack.getItem()).isInfinite(itemstack, stack, playerentity));
				if (!worldIn.isRemote) {
					BarenBlastItem arrowitem = (BarenBlastItem) (itemstack.getItem() instanceof BarenBlastItem
							? itemstack.getItem()
							: DoomItems.BARENBLAST.get());
					BarenBlastEntity abstractarrowentity = arrowitem.createArrow(worldIn, itemstack, playerentity);
					...
					if (flag1 || playerentity.abilities.isCreativeMode
							&& (itemstack.getItem() == DoomItems.BARENBLAST.get()
									|| itemstack.getItem() == DoomItems.BARENBLAST.get())) {
	...
	public BarenBlastEntity createArrow(World worldIn, ItemStack stack, LivingEntity shooter) {
		BarenBlastEntity arrowentity = new BarenBlastEntity(shooter, worldIn, ref.get());
		return arrowentity;
	}
	...
	public boolean isInfinite(ItemStack stack, ItemStack bow, net.minecraft.entity.player.PlayerEntity player) {
		...
		return enchant <= 0 ? false : this.getClass() == BarenBlastItem.class;
	}
